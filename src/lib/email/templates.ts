...
export function transferInvite(p: TransferInviteProps) {
  const subject = `Převod coworkingu ${p.coworkingName} na vás`;
  const html = layoutHtml(`
    <h1 style="font-size:22px;margin:0 0 16px;">Nabídka převodu správy</h1>
    <p>Dobrý den,</p>
    <p><strong>${p.fromName}</strong> na vás chce převést správu coworkingu <strong>${p.coworkingName}</strong> na coworkings.cz.</p>
    ${p.message ? `<p style="border-left:3px solid #1c1c1c;padding-left:16px;color:#5a5a5a;">${p.message}</p>` : ''}
    <p>Pro přijetí se přihlaste (nebo zaregistrujte) účtem s touto e-mailovou adresou a klikněte na tlačítko:</p>
    <p style="margin:24px 0;"><a href="${p.acceptUrl}" style="display:inline-block;padding:12px 24px;background:#1c1c1c;color:#fffdf8;text-decoration:none;font-weight:600;">Přijmout převod →</a></p>
    <p style="color:#5a5a5a;font-size:13px;">Odkaz platí ${p.expiresDays} dní. Pokud převod nečekáte, e-mail ignorujte.</p>
  `);
  const text = `${p.fromName} na vás chce převést správu coworkingu ${p.coworkingName} na coworkings.cz.
${p.message ? `\nZpráva: ${p.message}\n` : ''}
Přijmout převod: ${p.acceptUrl}
Odkaz platí ${p.expiresDays} dní.`;
  return { subject, html, text };
}
